# Taegyu Lim

Work with Tabular ML, Physical - Virtual alignment. 

## Topic of interests  
- Physical Informed Machine Learning
- Transfer Learning
- LLM based AI Agent
- Knowledge, Ontology & prediction model combination

## Career

Hankook Tire & Technology (2025.08 - )
- AI/ML based TPC (Tire Performance Characteristic), VPC (Vehicle Performance Characteristic) prediction
  SAE J2452 (GM, Tesla, Ford), Wet Braking, Hydroplaning

Leaders Associates Co. (2025.02 - 2025.07)
- Consumer reaction trend analysis with LLM + Embedding Model
- Development of a JavaScript-based web data collector

## Educations

M.S: Industrial & Management Engineering in Pohang University of Science and Technology (Supervisor: Prof. Duck-Young Kim)  
2023.02 - 2025.02
- Research Topic: Rapid Deep Learning based Transfer Learning for limited data

B.S: Mechanical Engineering & Physics in Yeungnam University
2017.02 - 2023.02
- National Presidental Science Scholarship (2021, Junior student)
